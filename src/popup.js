...
const renderPopupSettings = () => {
    ...
    chrome.storage.local.get(["autoShow", "displayLinks", "customUrls"], (result) => {
    ...
    <div>
        ...
        <div class="secondary-content">
            <a class="remove-custom-link" data-url-key="${customUrlKey}">
                <i class="material-icons" data-url-key="${customUrlKey}">delete</i>
            </a>
        </div>
    </div>
</li>`);
...
$(document.body).on("click", ".remove-custom-link", (event) => {
    ...
    chrome.storage.local.get(["customUrls"], (result) => {
        ...
        Materialize.toast("Removed your custom site successfully!", 3000, "blue darken-1 white-text");
...
$("#custom_torrent_form").on("submit", (event) => {
    ...
    const iconInput = $("#icon_url_custom").val().trim();
    const urlTemplate = $("#url_template_custom").val().trim();
    ...
    chrome.storage.local.get(["customUrls"], (result) => {
        ...
        Materialize.toast("Added your template successfully!", 3000, "blue darken-1 white-text");
